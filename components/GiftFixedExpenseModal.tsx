      ...
      <div className="bg-white dark:bg-gray-900 dark:border dark:border-gray-800 rounded-2xl shadow-2xl w-full max-w-md m-4 flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 space-y-4">
          <div className="p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
            <div className="flex items-center justify-between text-lg">
              ...
              <span className="font-mono font-semibold text-gray-500 dark:text-gray-400 line-through decoration-slate-400">{formatCurrency(expense.amount)}</span>
            </div>
            <p className="text-xs text-center text-gray-500 mt-2">
                Se registrará con costo 0 en tu historial.
            </p>
          </div>
           <div>
            <label htmlFor="gift-details" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nota (Opcional)</label>
            <textarea
                id="gift-details"
                value={details}
                onChange={(e) => setDetails(e.target.value)}
                rows={2}
                placeholder="Ej: Pagado por transferencia externa"
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-slate-500 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100"
            />
        </div>
          <div>
            <label htmlFor="gift-date" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fecha</label>
            <CustomDatePicker value={date} onChange={setDate} min={minDateForExpenses} themeColor="#64748b" displayMode="modal"/>
          </div>
        </div>
        ...
        <footer className="p-4 border-t border-gray-200 dark:border-gray-700 mt-auto">
          <button
            ...
            className="w-full bg-slate-500 text-white font-bold py-3 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-900 transition-colors hover:bg-slate-600"
          >
            Marcar como Pagado
          </button>
        </footer>
